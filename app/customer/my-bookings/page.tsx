...
export default function MyBookingsPage() {
  ...
  // --- Pricing Logic ---
  const FULL_FEE_PER_SLOT_PER_HOUR = 300; // Total cost rate
  const ONLINE_FEE_PER_SLOT = 150;        // Advance payment rate

  // 1. Calculate Full Total Cost
  const calculateTotal = (b: Booking) => 
    b.totalAmount ?? (FULL_FEE_PER_SLOT_PER_HOUR * b.duration * b.slots.length);

  // 2. Calculate Required Advance (The "Initial Payment")
  const calculateAdvance = (b: Booking) => 
    ONLINE_FEE_PER_SLOT * b.slots.length;
      ...
      <div className="flex flex-col lg:flex-row lg:items-end lg:justify-between gap-3 sm:gap-4">
        <div>
          ...
          <p className="text-xs sm:text-sm text-slate-400 mt-1">
            Pay <span className="text-lime-400 font-bold">Rs.{ONLINE_FEE_PER_SLOT}</span> per slot now. Pay the rest at the location.
          </p>
        </div>
